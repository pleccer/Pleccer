...
@ECHO                  console - run and wait on PrusaSlicer-console.exe
...
@ECHO                  window - run PrusaSlicer.exe (no wait)
@ECHO                Default: none
...
IF "%PS_RUN%" EQU "console" (
    icacls PrusaSlicer-console.exe >nul || GOTO :END
    start /wait /b PrusaSlicer-console.exe
) ELSE IF "%PS_RUN%" EQU "window" (
    icacls PrusaSlicer.exe >nul || GOTO :END
    start PrusaSlicer.exe
